...
QwtText ScrollZoomer::trackerTextF(const QPointF &pos) const
{
    ...
    if(*current){
        ...
        size_t size = (*current)->getSpect()->dataSize();
        if(x>=0 && x<(*current)->getSpect()->dataSize()){
            nText = QString("%1 (%2ch : %3)").
                    arg((*current)->getModel()->getSpectrumName()).
                    arg(QString::number(x)).
                    arg(QString::number((*current)->getSpect()->sample(x).toPoint().ry()));
        }
    } else {
...
/**
 * @brief SPRZoomerWithCorrel::SPRZoomerWithCorrel
 * @param _bases
 * @param parent
 * @param plot
 */
SPRZoomerWithCorrel::SPRZoomerWithCorrel(SPRSpectrumListItemsModel *_model, QWidget *parent, QwtPlot *plot):
    ScrollZoomer(parent, plot), spectModel(_model)
{
}

QwtText SPRZoomerWithCorrel::trackerTextF(const QPointF &pos) const
{
    QColor bg( Qt::white );
    bg.setAlpha( 200 );
    QwtText nText;
    ...
    if(spectModel){
        if(*current){
            ...
            SPRSpectrumItemModel *c = (*current)->getModel();
            if(c){
                uint8_t th = c->getThread();
                SPRSpectrumItemModel *b = spectModel->getBaseItemForChannel(th);
                double corr = 0;

                if(b){
                    corr = c->getCorrel(b);

                    nText = tr("%1 (%2:%3) корр.: %4").arg(c->getSpectrumName()).arg(x).arg(y).arg(QString::number(corr, 'f', 2));
                    nText.setBackgroundBrush(QBrush(bg));
                    return nText;
                }
            }
    ...
    return ScrollZoomer::trackerTextF(pos);
}
